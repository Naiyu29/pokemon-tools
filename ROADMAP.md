...
- [x] 4.2 端上辨識可行性 POC（2026-09-07 完成，見 `experiments/m4-recognition-poc/FINDINGS.md`）
      結論：外部圖庫模板比對不可行（姿勢不同）；採「遊戲原生模板庫自舉」——
      冷啟動用 Claude 辨識並自動累積模板，之後樸素比對、逐步趨近零成本
- [ ] 4.3 模板庫自舉機制實作（截圖裁切→存模板→比對優先、AI 回退）
- [ ] 4.4 懸浮視窗 overlay（需原生 app，工程量大，效益再評估）
