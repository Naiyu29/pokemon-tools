...
- 單打／雙打、天氣、順風、戲法空間切換
- **版面**：手機維持底部分頁列（一次一頁）；電腦（視窗 ≥1024px）自動換成
  桌面版工作區——左側導覽（對戰分析／隊伍／紀錄）＋「對手｜推薦｜傷害｜速度」
  多欄同畫面、整頁不捲、各欄獨立捲動（參考 Showdown damage calc 的排法）。
  ≥1600px 四欄全開；1280～1599px 中間欄上下各半（推薦／傷害）。
  鍵盤：`/` 跳到對手搜尋、`Enter` 收第一筆、`Esc` 清空
...
- `reports/weekly/`：對戰環境週報（每週搜尋當週 meta／賽事／新版本預告，比對威脅庫並給隊伍建議）；
  同名 `.html` 為手機版自包含網頁，複製到 `docs/reports/` 後可由 GitHub Pages 開啟
  （`https://naiyu29.github.io/pokemon-tools/reports/2026-W36.html`）
